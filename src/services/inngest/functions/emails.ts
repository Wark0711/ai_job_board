...
import { jobListAppTable, jobListTable, orgUserSettingsTable, userNotifSettingsTable } from "@/drizzle/schema";
...
export const prepareDailyOrganizationUserApplicationNotifications = inngest.createFunction(
    { id: "prepare-daily-organization-user-application-notifications", name: "Prepare Daily Organization User Application Notifications" },
    { cron: "TZ=America/Chicago 0 7 * * *" },
    async ({ step, event }) => {
        const getUsers = step.run("get-user-settings", async () => {
            return await db.query.orgUserSettingsTable.findMany({
                where: eq(orgUserSettingsTable.newApplicationEmailNotifications, true),
                columns: { userId: true, organizationId: true, newApplicationEmailNotifications: true, minimumRating: true },
                with: { user: { columns: { email: true, name: true } } },
            })
        })

        const getApplications = step.run("get-recent-applications", async () => {
            return await db.query.jobListAppTable.findMany({
                where: and(gte(jobListAppTable.createdAt, subDays(new Date(event.ts ?? Date.now()), 1))),
                columns: { rating: true },
                with: {
        ...
        const groupedNotifications = Object.groupBy(userNotifications, n => n.userId)
        ...
        const events = Object.entries(groupedNotifications).map(([, settings]) => {
            ...
            } as const satisfies GetEvents<typeof inngest>["app/email.daily-organization-user-applications"]
        }).filter(v => v != null)
