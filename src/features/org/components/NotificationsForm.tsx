...
import { RatingIcons } from "@/features/jobListApps/components/RatingIcons"
import { RATING_OPTIONS } from "@/features/jobListApps/data/constants"
import { updateOrganizationUserSettings } from "../actions/orgUserSettings"

export function NotificationsForm({ notificationSettings }: { notificationSettings?: Pick<typeof orgUserSettingsTable.$inferSelect, "newApplicationEmailNotifications" | "minimumRating"> }) {
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <FormItem>
                                <div className="flex items-center justify-between">
                                    <div className="space-y-0.5">
                                        ...
                                        <FormDescription>Receive summary emails of all new job listing applications</FormDescription>
                                    </div>
                                    <FormControl>
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <Select
                                            ...
                                            onValueChange={val => field.onChange(val === ANY_VALUE ? null : String(parseInt(val)))}
                                        >
                                            <FormControl>
                                                <SelectTrigger>
                                                    <SelectValue asChild>
                                                        {
                                                            field.value == null
                                                                ...
                                                                : <RatingIcons className="text-inherit" rating={field.value} />
                                                        }
                                                    </SelectValue>
                                                </SelectTrigger>
                                            </FormControl>
                                                ...
                                                <SelectItem value={ANY_VALUE}>Any Rating</SelectItem>
                                                {
                                                    RATING_OPTIONS.filter(r => r != null).map(rating => (
                                                        <SelectItem key={rating} value={rating.toString()}>
                                                            <RatingIcons className="text-inherit" rating={rating.toString()} />
                                                        </SelectItem>
                                                    ))
                                                }
                                            </SelectContent>
                ...
                <Button type="submit" disabled={form.formState.isSubmitting} className="w-full">
                    <LoadingSwap isLoading={form.formState.isSubmitting}>Save Notification Settings</LoadingSwap>
                </Button>
            </form>
        </Form>
    )
