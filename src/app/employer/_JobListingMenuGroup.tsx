...
type JobListing = Pick<typeof jobListTable.$inferSelect, "title" | "id"> & { applicationCount: number }

export function JobListingMenuGroup({ status, jobListings }: { status: JobListingStatus, jobListings: JobListing[] }) {

    const { jobListingId } = useParams()

    return (
        <SidebarMenu>
            <Collapsible defaultOpen={status !== "delisted" || jobListings.find(job => job.id === jobListingId) != null} className="group/collapsible">
                <SidebarMenuItem>
